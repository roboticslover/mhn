        ...
        <View style={styles.illustrationArea}>
          <View style={[styles.mainGlassCircle, { backgroundColor: c.cardElevated, borderColor: c.cardGlassBorder }]}>
            <View style={[styles.centerElement, { backgroundColor: c.accentSoft, borderColor: c.primary + '60' }]}>
              <Ionicons name="medkit-outline" size={32} color={c.primary} />
            </View>
          </View>
        </View>
        <Text style={[styles.emptyTitle, { color: c.text, fontFamily: 'Inter' }]}>No Vaccination{'\n'}Records Found</Text>
        <Text style={[styles.emptySubtitle, { color: c.textSecondary, fontFamily: 'Inter' }]}>
          Keep your vaccination records{'\n'}organized and accessible. Add your{'\n'}first record to get started.
        </Text>
        <TouchableOpacity style={[styles.addButton, { backgroundColor: c.accentSoft }]} activeOpacity={0.7} onPress={() => navigation.navigate('VaccineUpload')}>
          <Text style={[styles.addButtonText, { color: c.primary, fontFamily: 'Inter' }]}>ADD VACCINATION</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
